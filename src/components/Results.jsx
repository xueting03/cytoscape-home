...
const CardTitle = ({ logo, title, url }) => (
  <div className="flex items-center">
    {logo}
    <a href={url} target="_blank" rel="noreferrer" className="flex items-start group">
...
const Card = ({ logo, title, url, caption, isLoading, error, children, className }) => (
  <div
    className={`min-w-12 w-full flex-grow p-4 rounded-xl shadow-lg shadow-gray-200 ${error ? 'border-double border-4 border-red-100' : 'border border-gray-200'} ${className}`}
  >
    <CardTitle logo={logo} title={title} url={url} />
    <p className="mt-2 text-right text-xs text-gray-600">
      {!isLoading && !error ? <>{caption || <>&nbsp;</>}</> : <>&nbsp;</>}
    </p>
    <div className="mt-2 ring-4 ring-black ring-opacity-5 rounded-lg">{children}</div>
  </div>
);
...
const GeneManiaCard = ({ genes, organism }) => {
  ...
  return (
    <Card
      ...
      url={href}
      caption={
        data?.resultGenes?.length === 0 ? 'No results' : `${data?.resultGenes?.length} result genes`
      }
...
const NDExCard = ({ genes }) => {
  const { data, error, isFetching } = useQuery(createNDExQueryOptions(genes, genes?.length > 0));
  ...
  return (
    ...
    >
      <ul className="space-y-2 min-h-64">
        ...
        {results &&
          results.map(({ id, title, organisms, annotations, description, url, terms }, idx) => (
                ...
                <div className="mt-2 flex text-gray-400">
                  ...
                  <div>
                    <div className="text-xs text-gray-400">
                      <Marker
                        mark={terms}
                        options={{ className: 'bg-inherit font-bold text-inherit' }}
                      >
                        {annotations}
                      </Marker>
                    </div>
...
export function Results({ open = false, data, searchEngine, onClose }) {
  ...
  return (
    <Transition show={open}>
      <Dialog className="relative z-10 w-full" onClose={() => void 0 /**(make it modal)*/}>
                ...
                <div className="flex-initial md:px-4 px-2 md:pb-0 pb-2 bg-white border-b border-gray-200 drop-shadow-md">
                  <div className="flex flex-row sm:items-start items-center justify-center lg:space-x-10 md:space-x-6 space-x-4 max-w-7xl mx-auto md:pt-5 pt-2 text-gray-400">
                    <a aria-label="Home" onClick={onClose} className="cursor-pointer">
                      <Button
                        variant="text"
                        className="mt-2 text-gray-900 hover:text-complement-500 sm:hidden"
                      >
                    ...
                    <div className="w-full max-w-4xl text-left">
                      ...
                      <div className="mt-2 text-sm text-gray-400 lg:text-left text-right hidden md:block">
                        <span className="lg:inline hidden">
                          This is a small demonstration of the Cytoscape ecosystem.&nbsp;&nbsp;
                        </span>
                        <a
                          onClick={handleWhatElseClick}
                          className="text-complement-400 hover:underline cursor-pointer"
                        >
