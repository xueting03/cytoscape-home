...
export function GeneWizard({
  ...
  const genes = useMemo(() => (searchText ? parseGeneList(searchText) : []), [searchText]);

  const genesRef = useRef([]);
  const orgRef = useRef();

  const { data: taxidCounts, isFetching } = useQuery(
  ...
  useEffect(() => {
    ...
    if (validTaxids?.length > 0) {
      ...
      setInitialOrganismIndex(
        filteredOrganisms.findIndex(org => org.taxon === orgRef.current?.taxon)
      );
    } else {
  ...
  return (
    <div className="min-h-48">
      ...
      {!isFetching && step === 1 && (
        <OrganismsPanel
          organisms={organisms}
          initialSelectedIndex={initialOrganismIndex}
          onChange={handleOrganismsChange}
        />
      )}
    </div>
  );
